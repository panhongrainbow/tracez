...
func Test_Check_borrowFromBottomIndexNode_Function(t *testing.T) {
	...
	t.Run("Scenario in Chapter 2.3.4", func(t *testing.T) {
		...
		deleted, _, _, _ = basicDeletionBpTree.RemoveValue(BpItem{Key: 9}) // ⚔️ Delete crucial data and test.
	...
	t.Run("Scenario in Chapter 2.3.6", func(t *testing.T) {
		...
		// Check the contents of the data in the 1st index node.
		require.Equal(t, 1, len(basicDeletionBpTree.root.IndexNodes[1].DataNodes[2].Items))
		require.Equal(t, int64(11), basicDeletionBpTree.root.IndexNodes[1].DataNodes[2].Items[0].Key)
		...
		// Check the index node of the first level after deleting data.
		require.Equal(t, []int64{7, 11}, basicDeletionBpTree.root.Index)
		...
		// Check the contents of the data in the 2nd index node.
		require.Equal(t, int64(7), basicDeletionBpTree.root.IndexNodes[1].DataNodes[0].Items[0].Key)
		require.Equal(t, int64(8), basicDeletionBpTree.root.IndexNodes[1].DataNodes[0].Items[1].Key)
		require.Equal(t, int64(9), basicDeletionBpTree.root.IndexNodes[1].DataNodes[1].Items[0].Key)
		require.Equal(t, int64(10), basicDeletionBpTree.root.IndexNodes[1].DataNodes[1].Items[1].Key)
